...
At the start of coding, debugging, audit, planning, architecture, implementation, test, refactor, or repo-investigation requests, consider the `auto-workflow-router` skill first. Use it to choose the right Superpowers skill, OMX role/workflow, existing Codex skill, or direct response path when the user does not explicitly name one.
<!-- END AUTO WORKFLOW ROUTER -->
