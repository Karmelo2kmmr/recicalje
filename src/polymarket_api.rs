...
impl PolymarketAPI {
                ...
                Ok(resp) => {
                    if resp.status().is_success() {
                        if let Ok(m) = resp.json::<serde_json::Value>().await {
                            ...
                            let tokens: Vec<Token> = token_ids.iter().enumerate().map(|(i, id)| {
                                Token {
                                    token_id: id.clone(),
                                    outcome: if i == 0 { "Up".to_string() } else { "Down".to_string() },
                                    price: prices.get(i).and_then(|p| p.parse().ok()).unwrap_or(0.5),
                                }
                            }).collect();

                            info!("🎯 Predicted 5m Market Detected: {} | ID: {}", question, m["id"]);
                        ...
                        warn!("⚠️ Current 5m slug not found: {} (Status: {})", slug, resp.status());
                    }
                }
                Err(e) => { error!("Error fetching slug {}: {}", slug, e); }
            }
        }
        filtered
    }
    ...
    pub async fn place_order(&self, token_id: &str, price: f64, amount: f64, side: &str) -> bool {
        let is_paper = std::env::var("PAPER_TRADING").unwrap_or_else(|_| "true".to_string()) == "true";
        
        if is_paper {
            info!("📋 [PAPER] {} order on {} at {:.4}: ${:.2}", side, token_id, price, amount);
            true
        } else {
    ...
    pub async fn get_market_outcome(&self, market_id: &str) -> Option<String> {
        ...
        match self.client.get(&url).send().await {
            ...
            Err(e) => { error!("Error fetching market outcome {}: {}", market_id, e); }
        }
        None
    }
}
